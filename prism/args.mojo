...
alias ArgValidatorFn = fn (String, List[String]) -> Optional[String]


fn no_args(command_name: String, args: List[String]) -> Optional[String]:
    ...
    """
    if len(args) > 0:
        return sprintf("The command `%s` does not take any arguments.", command_name)
    return None


fn arbitrary_args(command_name: String, args: List[String]) -> Optional[String]:
...
fn minimum_n_args[n: Int]() -> ArgValidatorFn:
    ...
    fn less_than_n_args(command_name: String, args: List[String]) -> Optional[String]:
        if len(args) < n:
            return sprintf(
...
fn maximum_n_args[n: Int]() -> ArgValidatorFn:
    ...
    fn more_than_n_args(command_name: String, args: List[String]) -> Optional[String]:
        if len(args) > n:
            return sprintf("The command `%s` accepts at most %d argument(s). Received: %d.", command_name, n, len(args))
        return None
...
fn exact_args[n: Int]() -> ArgValidatorFn:
    ...
    fn exactly_n_args(command_name: String, args: List[String]) -> Optional[String]:
        if len(args) != n:
            return sprintf("The command `%s` accepts exactly %d argument(s). Received: %d.", command_name, n, len(args))
        return None
...
fn valid_args[valid: List[String]]() -> ArgValidatorFn:
    ...
    fn only_valid_args(command_name: String, args: List[String]) -> Optional[String]:
        if len(valid) > 0:
            for arg in args:
                if not contains(valid, arg[]):
                    return sprintf("Invalid argument: `%s`, for the command `%s`.", arg[], command_name)
        return None
...
fn range_args[minimum: Int, maximum: Int]() -> ArgValidatorFn:
    ...
    fn range_n_args(command_name: String, args: List[String]) -> Optional[String]:
...
fn match_all[arg_validators: List[ArgValidatorFn]]() -> ArgValidatorFn:
    ...
    fn match_all_args(command_name: String, args: List[String]) -> Optional[String]:
        for i in range(len(arg_validators)):
            var error = arg_validators[i](command_name, args)
            if error:
                return error
        return None
