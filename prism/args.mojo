...
fn no_args(command: Command, args: List[String]) -> Optional[String]:
    ...
    """
    if len(args) > 0:
        return sprintf("The command `%s` does not take any arguments.", command.name)
    return None


fn arbitrary_args(command: Command, args: List[String]) -> Optional[String]:
...
fn minimum_n_args[n: Int]() -> ArgValidator:
    ...
    fn less_than_n_args(command: Command, args: List[String]) -> Optional[String]:
        if len(args) < n:
            return sprintf(
...
fn maximum_n_args[n: Int]() -> ArgValidator:
    ...
    fn more_than_n_args(command: Command, args: List[String]) -> Optional[String]:
        if len(args) > n:
            return sprintf("The command `%s` accepts at most %d argument(s). Received: %d.", command.name, n, len(args))
        return None
...
fn exact_args[n: Int]() -> ArgValidator:
    ...
    fn exactly_n_args(command: Command, args: List[String]) -> Optional[String]:
        if len(args) != n:
            return sprintf("The command `%s` accepts exactly %d argument(s). Received: %d.", command.name, n, len(args))
        return None
...
fn valid_args[valid: List[String]]() -> ArgValidator:
    ...
    fn only_valid_args(command: Command, args: List[String]) -> Optional[String]:
        if len(valid) > 0:
            for arg in args:
                if not contains(valid, arg[]):
                    return sprintf("Invalid argument: `%s`, for the command `%s`.", arg[], command.name)
        return None
...
fn range_args[minimum: Int, maximum: Int]() -> ArgValidator:
    ...
    fn range_n_args(command: Command, args: List[String]) -> Optional[String]:
...
fn match_all[arg_validators: List[ArgValidator]]() -> ArgValidator:
    ...
    fn match_all_args(command: Command, args: List[String]) -> Optional[String]:
