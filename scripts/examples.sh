...
# pixi run mojo build -D ASSERT=all $TEMP_DIR/aliases.mojo -o $TEMP_DIR/aliases
# pixi run mojo build -D ASSERT=all $TEMP_DIR/hello_world.mojo -o $TEMP_DIR/hello_world
# pixi run mojo build -D ASSERT=all $TEMP_DIR/fg_parent.mojo -o $TEMP_DIR/parent
# pixi run mojo build -D ASSERT=all $TEMP_DIR/fg_child.mojo -o $TEMP_DIR/child
# pixi run mojo build -D ASSERT=all $TEMP_DIR/arg_validators.mojo -o $TEMP_DIR/validators
# pixi run mojo build -D ASSERT=all $TEMP_DIR/alt_flag_values.mojo -o $TEMP_DIR/alt_flag_values
# pixi run mojo build -D ASSERT=all $TEMP_DIR/flag_action.mojo -o $TEMP_DIR/flag_action
# pixi run mojo build -D ASSERT=all $TEMP_DIR/list_flags.mojo -o $TEMP_DIR/list_flags
# pixi run mojo build -D ASSERT=all $TEMP_DIR/version.mojo -o $TEMP_DIR/version
# pixi run mojo build -D ASSERT=all $TEMP_DIR/exit.mojo -o $TEMP_DIR/exit
# pixi run mojo build -D ASSERT=all $TEMP_DIR/multiple_bool_flag.mojo -o $TEMP_DIR/multiple_bool_flag
# pixi run mojo build -D ASSERT=all $TEMP_DIR/suggest.mojo -o $TEMP_DIR/suggest
# pixi run mojo build -D ASSERT=all $TEMP_DIR/full_api.mojo -o $TEMP_DIR/full_api

cp $TEMP_DIR/$PACKAGE_NAME.mojopkg $TEMP_DIR/printer/$PACKAGE_NAME.mojopkg
cp $TEMP_DIR/$PACKAGE_NAME.mojopkg $TEMP_DIR/requests/$PACKAGE_NAME.mojopkg
pixi run mojo build -D ASSERT=all $TEMP_DIR/printer/printer.mojo -o $TEMP_DIR/printer
# cd ../requests
# pixi run mojo build -D ASSERT=all $TEMP_DIR/requests/nested.mojo -o $TEMP_DIR/nested
# pixi run mojo build -D ASSERT=all $TEMP_DIR/requests/persistent_flags.mojo -o $TEMP_DIR/persistent_flags
...
# $TEMP_DIR/nested get cat --count 3 -l
# $TEMP_DIR/persistent_flags get cat --count 2 --lover
# $TEMP_DIR/persistent_flags get dog
