...
struct Scanner[R: io.Reader]():
    ...
    fn current_token_as_bytes(self) -> Bytes:
    ...
    fn buffer(inout self, buf: Bytes, max: Int) raises:
...
# TODO: For now, passing in token and err to be modified by the SplitFunction. This is because the Tuple return unpacking cannot handle a memory only type (Bytes and Err)
